...
def test_checkout_lines_add_custom_price_discounted(
    app_api_client, checkout, stock, permission_handle_checkouts
):
    variant = stock.product_variant
    variant_id = graphene.Node.to_global_id("ProductVariant", variant.pk)
    ...
    variables = {
        "id": to_global_id_or_none(checkout),
        "lines": [{"variantId": variant_id, "quantity": 1, "priceDiscounted": price}],
        "channelSlug": checkout.channel.slug,
    }
    response = app_api_client.post_graphql(
        MUTATION_CHECKOUT_LINES_ADD,
    ...
    data = content["data"]["checkoutLinesAdd"]
    assert not data["errors"]
    line = data["checkout"]["lines"][0]
    assert Decimal(line["unitPrice"]["gross"]["amount"]) == 4.42
    assert Decimal(line["totalPrice"]["gross"]["amount"]) == 4.42
