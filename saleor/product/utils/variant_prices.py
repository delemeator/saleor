...
def _update_variant_channel_listings(changed_variants_listings_to_update):
    with transaction.atomic():
        rule_ids = {
            listing.applied_rule_id
            for listing in changed_variants_listings_to_update
        ...
        for listing in changed_variants_listings_to_update:
            if (
                listing.applied_rule_id
                and listing.applied_rule_id not in valid_rule_ids
            ):
                listing.applied_rule = None
...
def _update_or_create_listings(
    ...
    if changed_variants_listings_to_update:
        _update_variant_channel_listings(changed_variants_listings_to_update)
        # ProductVariantChannelListing.objects.bulk_update(
        #     sorted(changed_variants_listings_to_update, key=lambda listing: listing.id),
        #     ["discounted_price_amount", "applied_rule"],
        # )
    ...
    for variant_listing_id in sorted(variant_listing_cleanup_rules):
        rule_id = variant_listing_cleanup_rules[variant_listing_id]

        qs = VariantChannelListingPromotionRule.objects.filter(
            variant_channel_listing_id=variant_listing_id
        )
...
def _get_discounted_variants_prices_for_promotions(
    ...
    for variant_listing in variant_listings:
        ...
        if variant_listing.discounted_price != discounted_variant_price or (
            ...
            variant_listing_cleanup_rules[variant_listing.id] = rule_id
